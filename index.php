...
if (isset($_GET['verification'])) {
    ...
    $stmt1->execute([$code]);
    $user = $stmt1->fetch(PDO::FETCH_ASSOC); // get user's data

    $stmt = $conn->prepare("UPDATE acc_inf SET is_verified = ? , is_applicant = ? WHERE ver_code = ?");
    $stmt->execute([$status, $status, $code]);
    $msg = "<div class='alert alert-success'>You're account is now Verified. Please proceed to login in.</div>";
}
        ...
        if ($pass == $password) {
            if ($is_verified == '0') {
                ...
                if ($is_applicant == '0') {
                    ...
                    //get status of profile information. If it is not set, proceed to acc_inf.php to insert needed information
